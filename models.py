...
class Database:
    ...
    def create_tables(self):
        ...
        # Backwards compatibility: ensure apartment_number column exists on existing DBs
        try:
            self.cursor.execute("PRAGMA table_info(locations)")
            cols = [row[1] for row in self.cursor.fetchall()]
            if "apartment_number" not in cols:
                self.cursor.execute("ALTER TABLE locations ADD COLUMN apartment_number TEXT")
        except sqlite3.OperationalError:
            # If PRAGMA or ALTER fails, continue; main table still exists
            pass
        ...
        # Service Orders table (service_number is optional and may be reused)
        self.cursor.execute("""
        ...
        # Backwards compatibility: ensure new service_* columns exist on existing DBs
        try:
            self.cursor.execute("PRAGMA table_info(transactions)")
            cols = [row[1] for row in self.cursor.fetchall()]
            if "service_address" not in cols:
                self.cursor.execute("ALTER TABLE transactions ADD COLUMN service_address TEXT")
            if "service_apartment" not in cols:
                self.cursor.execute("ALTER TABLE transactions ADD COLUMN service_apartment TEXT")
        except sqlite3.OperationalError:
            pass
    ...
    def add_location(self, name: str, address: str, apartment_number: str = "") -> int:
        """Add a new location"""
        try:
            self.cursor.execute("INSERT INTO locations (name, address, apartment_number) VALUES (?, ?, ?)", 
    ...
    def get_all_locations(self) -> List[Tuple]:
        """Get all locations"""
        self.cursor.execute("SELECT location_id, name, address, apartment_number FROM locations ORDER BY name")
        return self.cursor.fetchall()
    
    def update_location(self, location_id: int, name: str, address: str, apartment_number: str = ""):
        """Update location details"""
        self.cursor.execute("UPDATE locations SET name = ?, address = ?, apartment_number = ? WHERE location_id = ?",
                          (name, address, apartment_number, location_id))
        self.conn.commit()
    ...
    def add_service_order(self, service_number: Optional[str], address: str, 
    ...
    def get_service_order_by_number(self, service_number: str) -> Optional[Tuple]:
        """Get the most recent service order with a given number (if any)"""
        self.cursor.execute("""
    ...
    def add_transaction(self, item_id: int, technician_id: int, location_id: int,
        ...
        self.cursor.execute("""
            ...
            (item_id, technician_id, service_id, location_id, action_type, 
             quantity, price, timestamp, service_address, service_apartment)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (item_id, technician_id, service_id, location_id, action_type,
              quantity, price, timestamp, service_address, service_apartment))
    ...
    def get_all_transactions(self, filters: Dict = None) -> List[Dict]:
        ...
        query = """
            SELECT t.transaction_id, t.timestamp, t.action_type, t.quantity, t.price,
                   ...
                   t.service_address, t.service_apartment
            FROM transactions t
        ...
        for row in self.cursor.fetchall():
            transactions.append({
                ...
                'location_name': row[8],
                'service_address': row[9] or "",
                'service_apartment': row[10] or "",
            })

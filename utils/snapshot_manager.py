...
class ValidationHelper:
    ...
    @staticmethod
    def validate_apartment_number(value: str) -> str:
        """Validate apartment number format (no spaces or slashes)"""
        if not value:
            return value  # Empty is allowed
        
        if ' ' in value or '/' in value:
            raise ValueError("Apartment number cannot contain spaces or slashes")
        
        return value
    
    @staticmethod
    def validate_service_number(value: str) -> str:
        """Validate service number format (12345-6: 5 digits, dash, number 1-20)"""
        if not value:
            return value  # Empty is allowed
        ...
        if not re.match(pattern, value):
            raise ValueError("Service number must be in format 12345-6 (5 digits, dash, number)")
        
        # Check that the number after dash is 1-20
        parts = value.split('-')
        num = int(parts[1])
        if num < 1 or num > 20:
            raise ValueError("Service number suffix must be between 1 and 20")
...
class SoundManager:
    ...
    def _load_sounds(self):
        ...
        success_path = os.path.join(root_dir, "song519_saveSound.wav")
        if os.path.exists(success_path):
            self.success_sound = QSoundEffect()
            self.success_sound.setSource(QUrl.fromLocalFile(success_path))
            self.success_sound.setVolume(0.5)
        
        error_path = os.path.join(root_dir, "song285_errorSound.wav")
